...
module moon {
	...
	export class Skin {
		...
		/**默认按钮 */
		public static get buttonNormal(): Sprite { return MoonUI.getRect(60, 60, Color.skinNormal) };
		public static get buttonDown(): Sprite { return MoonUI.getRect(60, 60, Color.skinDown) };
		/**默认单选框 */
	...
	export class MoonUI {
		...
		public static getSwitch(bc: number = 0XFFFFFF, gc: number = 0, type: number = 0): Sprite {
			var node: Sprite = this.getRoundRect(80, 50, bc, 60, 60);
	...
	export class LogManager {
		...
		public static getInstane(): LogManager {
	...
	export class MoonContainer extends DisplayObjectContainer {
		...
		protected stageW: number;
		protected stageH: number;
		public constructor() {
			super();
			this.once(egret.Event.ADDED_TO_STAGE, this.render, this);
		}
		/**加载到舞台之后调用 */
		...
		public dispEvent(type: string, data: Object = null, dataType: Object = null): void {
			if (!this.dataEvent)
				return;
			var listenerFunc: Function = this.dataEvent[type] as Function;
			...
			if (listenerFunc["this"]) {
				(<Function>listenerFunc).apply(listenerFunc["this"], [moonEvent]);
			} else {
		...
		public removeChildAll(): void {
			let beginIndex: number = 0, endIndex = this.numChildren - 1;
			for (var i: number = beginIndex; i <= endIndex; ++i) {
				let display: DisplayObject = this.getChildAt(0);
	...
	export class BasicView extends MoonContainer {
		...
		protected createRect(w: number, h: number, c: number = 0, x: number = 0, y: number = 0): Sprite {
			var sprite: Sprite = MoonUI.getRect(w, h, c, x, y);
	...
	export class GameView extends BasicView {
		...
		protected createButton(name: string, x: number = 0, y: number = 0): BasicButton {
	...
	*/
	...
	export class SliderBar extends ProgressBar implements ILayout {
		...
		get value(): number {//get 方法竟然是不能继承的，这个得注意了。
			return this._value;
		}
		/**横竖版布局，默认是横版布局 */
	...
	export class AlertBar extends BasicBar {
		...
		protected initView(): void {
			...
			var w: number = tw + 80;
			var h: number = th + 120;
			var x: number = (this.stageW - w) >> 1;
			var y: number = (this.stageH - h) >> 1;
	...
	export class PanelBar extends BasicBar {
		...
		get windowRect(): Rectangle {
			var rect: Rectangle = new Rectangle(0, 0, this.stageW, this.stageH);
			return rect;
		}
	...
	export class GameLoad extends moon.GameView {
		...
		protected render(): void {
			...
			var sw: number = this.stageW;
			var sh: number = this.stageH;
			var w: number = 80;
