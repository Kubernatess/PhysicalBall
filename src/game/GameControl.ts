...
class GameControl extends moon.GameView {
    private P2Simulation: P2Simulation;//2D物理世界
    ...
    private readonly minBodyCount: number = 1;//数字刚体的最少数值
    ...
    protected render(): void {
        super.render();
        // trace("v1.1");
        GameData.stageH = this.stageH;
        GameData.stageW = this.stageW;

        let soundController: SoundControl = SoundControl.getIns();
        soundController.addItem(MUSIC_HIT);
        soundController.addItem(MUSIC_ADDNUM);
        soundController.addItem(MUSIC_FIRE);
        soundController.addItem(MUSIC_FLOOR);
        soundController.addItem(MUSIC_BG, true);
        //soundController.play(MUSIC_BG, 0, 9999);

        this.addChild(new MImage("bg_jpg"));

        var world: P2Simulation = new P2Simulation(0, 200);
        var walls: p2.Body[] = world.createWall(new egret.Rectangle(0, 0, GameData.stageW, GameData.stageH));

        //this.addChild(world);
        this.P2Simulation = world;
        ...
        var pipeHight: number = left.displays[0].height / 2;
        var pipeY: number = GameData.stageH - pipeHight - 100;
        left.position = [25, pipeY];
        right.position = [GameData.stageW - 25, pipeY];
        ...
        top.position = [GameData.stageW >> 1, 0];
        ...
        var conMask: Sprite = moon.MoonUI.getRect(GameData.stageW, GameData.stageH - 200, 0, 0, 160);
    ...
    protected testZero(): void {
        for (var i: number = 0; i < 6; i++) {
            ...
            var skin: NumImage = body1.displays[0];
            skin.initNum(100);
            body1.angle = 45;
    ...
    protected moveNumBody(): void {
        var bodys = this.P2Simulation.p2World.bodies;
        var l: number = bodys.length;
        for (var i: number = 0; i < l; i++) {
            var body: p2.Body = bodys[i];
            if (body.displays) {
                if (body.userData && body.userData["num"]) {
                    body.position[1] -= 10;
                    if (body.position[1] <= OVER_DIS) {
                        if (body.displays[0].imageName != ICON_ADD) {
                            this.over();
                            return;
                        }
                    }
    ...
    protected over(): void {
        ...
        for (var i: number = 0; i < bodys.length; i++) {
            ...
            if (body.displays) {
                if (body.userData && body.userData["num"]) {
                    var skin: MImage = body.displays[0];
                    this.P2Simulation.removeBodys.push(body);
    ...
    protected updateNumBody(): void {
        ...
        for (var i: number = 0; i < 5; i++) {
            if (Math.random() < 0.7) {
                ...
                var skin: NumImage = body.displays[0];
    ...
    protected onHitBegin(evt): void {
        ...
        if (evt.bodyA.userData["ball"] && evt.bodyB.userData["pipe"] || evt.bodyB.userData["ball"] && evt.bodyA.userData["pipe"]) {
            ...
            if (numBody) {
                this.score++;
                this.updateScore();
                var skin: NumImage = numBody.displays[0];
                ...
                if (skin.value == 0) {
                    ...
                    this.P2Simulation.removeBodys.push(numBody);
                    skin.removeFromParent(true);
                } else {
                    var vec = new Point(ball.velocity[0], ball.velocity[1]);
                    var vecnum: number = Math.sqrt(vec.x * vec.x + vec.y * vec.y);
        ...
        if (evt.bodyA.userData["ball"] && evt.bodyB.userData["floor"] || evt.bodyB.userData["ball"] && evt.bodyA.userData["floor"]) {
            ...
            this.removeSkins.push(ball.displays[0]);
            this.P2Simulation.removeBodys.push(ball);
        }
    }
    ...
    private getBottom(distance: number): number {
        return GameData.stageH - distance;
    }
    /** 传入宽后居中距离*/
    private getCenterX(skinWidth: number): number {
        return (GameData.stageW - skinWidth) >> 1;
    }
    /**设置与球的碰撞弹性 */
    private doMaterial(ball: p2.Body): void {
        var world: P2Simulation = this.P2Simulation;
    ...
    private setMaterial(body1: p2.Body, body2: p2.Body): void {
        ...
        this.P2Simulation.p2World.addContactMaterial(roleAndStoneMaterial);
    }
    /** 创建底座三角形刚体*/
    ...
    protected loop(n: number): boolean {
        ...
        for (var i: number = 0; i < skins.length; i++) {
            ...
            skin.y -= 20;
            if (skin.x > GameData.stageW / 2) {
                skin.x = GameData.stageW - skin.width;
            } else {
                skin.x = skin.width;
            }
...
class NumImage extends MImage {
    ...
    public initNum(v: number): void {
        this.txt = (new moon.Label).textField;
